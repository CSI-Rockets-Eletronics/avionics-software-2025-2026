    ...
    void ParseCommand() {
        ...
        switch (command_packet.command) {
            ...
            case FsCommand::STATE_ENGINE_PRIME:
                // ENGINE_PRIME: Transition to EREG_STAGE_1
                divergence_latched_ = false;
                SetState(EREG_STAGE_1);
                BumplessResetPID();
                Serial.println("EREG: ENGINE_PRIME command received - entering STAGE_1");
                break;
            case FsCommand::STATE_FIRE:
                // FIRE: Transition to EREG_STAGE_2
                divergence_latched_ = false;
                SetState(EREG_STAGE_2);
                Serial.println("EREG: FIRE command received - entering STAGE_2");
                break;
