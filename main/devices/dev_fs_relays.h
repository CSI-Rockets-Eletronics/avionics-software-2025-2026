...
class DevFsRelays : public Device {
   private:
    ...
    const MS kFireIgniterOnDelayMs = 3000;            // Igniter fires immediately
    const MS kFireIgniterOffDelayMs = 3500;         // 500ms pulse
    const MS kFireRunOpenDelayMs = 10000;           // Wait 10s after igniter on, then open run (T-0:00)
    const MS kFireBackToStandbyDelayMs = 30000;    // 30s total
