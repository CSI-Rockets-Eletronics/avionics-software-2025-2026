...
// AV Bay Relay pins - controlling Pilot, Run, and Ereg Power relays
// These relays were moved from dev_fs_relays.h to this new AV bay node

enum class AvRelayPin : int {
    PRESS_PILOT = 21,  // Dome pilot valve
    RUN = 8,           // Main run valve
    EREG_POWER = 41,   // Ereg power control
};

struct AvRelayStates {
...
class DevAvRelays : public Device {
   private:
    // Timing constants from original FsRelays implementation
    ...
    AvRelayStates relay_states;

   public:
    void Setup() override {
        SetPinToOutput(AvRelayPin::PRESS_PILOT);
        SetPinToOutput(AvRelayPin::RUN);
        SetPinToOutput(AvRelayPin::EREG_POWER);
    }
    ...
    void ParseCommand() {
        ...
        Serial.print("[AV RELAYS] Received FsCommandPacket, command: ");
        Serial.println(static_cast<int>(command_packet.command));

        if (command_packet.command == FsCommand::RESTART) {
            Serial.println("[AV RELAYS] RESTART command received - rebooting!");
        ...
        Serial.print("[AV RELAYS] Entered state: ");
    ...
    void UpdateRelayStates() {
        ...
        switch (cur_state) {
            ...
            case FsState::GN2_PULSE_FILL_C:
                // no AV relays to set in these states
                break;
            case FsState::FIRE:
                if (time_in_state < kFireDomePilotCloseDelayMs) {
                    ...
                    // igniter relay is on FsRelays node
    ...
    void FlushRelays() {
        FlushRelay(AvRelayPin::PRESS_PILOT, relay_states.press_pilot);
        FlushRelay(AvRelayPin::RUN, relay_states.run);
        FlushRelay(AvRelayPin::EREG_POWER, relay_states.ereg_power);
    }

    void SetPinToOutput(AvRelayPin pin) {
        pinMode(static_cast<int>(pin), OUTPUT);
    }

    void FlushRelay(AvRelayPin pin, bool state) {
        digitalWrite(static_cast<int>(pin), state ? HIGH : LOW);
    }

    void SendState() {
        AvRelayStatePacket state_packet{
        ...
        // Send state to PacketForwarder on fs_scientific1 via ESP-NOW
        Send(DeviceType::DevFsPacketForwarder, state_packet);
    }
};

REGISTER_AVIONICS_DEVICE(DevAvRelays);
